...
namespace FacultativosWebApi.Jwt
{
    public static class JwtManager
    {
        ...
        /// </summary>5b834700-2f8a-4622-bb51-9eb80a5f52dd
        private const string Secret = "";

        public static object AuthenticationTypes { get; private set; }
        ...
        public static ClaimsPrincipal GetPrincipal(string token)
        {
            try
            {
                //EL módulo y exponente se obtienen de la petición http://ServidorKeycloak/auth/realms/NombreRealm/protocol/openid-connect/certs
                //Donde ServidorKeycloak es la ip del servidor que aloja Keycloak y NombreRealm es el realm creado en ese servidor.
                RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
                rsa.ImportParameters(
                  new RSAParameters()
                  {
                      Modulus = FromBase64Url("u_1pNGKsQHrliyDYwgpKgjbHqloim0yvI-zJixk4xeMWIY6NxqQ5FSUbOuT9ecnc6S1zkALTdY6BVug9G86w4XZibroLTQxvNZqbtotBcXVMqhWc1WBeu2XwOLnZgeyfIHz-zl4-5C48zpTgD5u_ZJ05IbOdr0lpvW0FIOpFSXdjdEF5wE3dfsBH2HuPmPKNndibA3XCkJgE9CtFhvZjwzQDrGH5UXNQRihi0BDx66XRdaCeotbtG7rRuU9_CGVoeMltf_g22u-IRvp05uvNCnoGqFdi-nAL1sYoFWHNufA0PPiZgZn6tbuq4r5ksom9miYyL6SDvliho2Dlxljlew"),
        ...
        public static ClaimsIdentity CreateIdentity(ClaimsPrincipal simplePrinciple)
        {
            try
            {
                ...
                foreach (var claim in simplePrinciple.Claims)
                {
                    switch (claim.Type)
                    {
                        case "preferred_username":
                            ...
                            break;
                        case "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/given_name":
                            identity.AddClaim(new Claim(ClaimTypes.GivenName, claim.Value));
                            break;
                        case "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname":
                            identity.AddClaim(new Claim(ClaimTypes.Surname, claim.Value));
                            break;
                        case "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress":
                            identity.AddClaim(new Claim(ClaimTypes.Email, claim.Value));
                            break;
                        case "resource_access":
                            var Cuentas = JsonConvert.DeserializeObject<Dictionary<string, Cuentas>>(claim.Value);
                            var Cuenta = Cuentas.FirstOrDefault(kvp => kvp.Key.Contains(WebConfigurationManager.AppSettings["Cliente"]));

                            foreach (string rol in Cuenta.Value.roles.ToList())
                            {
        ...
        {
            public const string IDX00000 = "Invalid token.";
